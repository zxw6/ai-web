# Spring Boot 2 + WebClient + JWT GPT Mirror Plan
...
1. Frontend sends the user's message to `/api/chat/stream`.
2. Backend identifies the current user from the JWT.
3. Backend validates that the conversation belongs to the current user.
4. Backend saves the user message.
5. Backend calls OpenAI with `WebClient`.
6. Backend forwards streamed chunks to the frontend with `SseEmitter`.
7. Backend saves the completed assistant message.
